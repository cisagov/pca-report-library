...
def graph_builder(assessment_ID, labels):
    ...
    if success:
        ...
        with open('reportData_' + reportData['RVA_Number'] + '.json',
                  'w') as fp:
...
def unique_user_click_rate_vs_report_rate_per_level_deception(reportData):
    ...
    if userReportProvided:
        ...
        for numlevel, value1 in reportData["Level"].items():
            report_rate.append(round(
                reportData['Level'][numlevel]['User_Reports'] /
                reportData['Level'][numlevel]['Emails_Sent_Attempted'] * 100,
                2))
        ...
        for numlevel, value1 in reportData["Level"].items():
            if reportData['Level'][numlevel]['User_Clicks'] == 0:
                ...
                ratio.append(round(
                    reportData['Level'][numlevel]['User_Reports'] /
                    reportData['Level'][numlevel]['User_Clicks'], 1))
    ...
    rects1 = plt.bar(index, click_rate, bar_width,
                     color=BLUE,  # blue
                     label='Unique Click Rate')
    rects1_lable_buffer = plt.bar(index, 3, bar_width,
                                  bottom=click_rate, edgecolor="none",
                                  color='none')
    
    # Establishes the User Report Rate bars
    rects2 = plt.bar(index + bar_width, report_rate, bar_width,
                     color=DARK_RED,  # dark red
                     label='User Report Rate')
    rects2_label_buffer = plt.bar(index + bar_width, 3, bar_width,
                                  bottom=report_rate, edgecolor="none",
                                  color='none')
    ...
    if userReportProvided and max(ratio) <= 2.0:
        plt.legend(loc='upper center', bbox_to_anchor=(.30, -0.2),
        ...
        ratio_text = [
            axes2.text(x_val, ratio_val + 0.05, ratio_val, ha="center",
                       va="bottom", fontsize=8) for x_val, ratio_val in
            zip(ratio_x, ratio)]
        
        to_avoid = rects1_lable_buffer + rects2_label_buffer
        adjust_text(ratio_text, add_objects=to_avoid,
                    only_move={'points': 'y', 'text': 'y', 'objects': 'y'},
                    va='top', autoalign="y")
        
        # Adjusts the legend location
        plt.legend(loc='upper center', bbox_to_anchor=(.78, -0.2),
        ...
        plt.legend(loc='upper center', bbox_to_anchor=(0.50, -0.2),
                   frameon=False, ncol=2)
    ...
    plt.tight_layout()
    plt.savefig('figures/UniqueUserClickRateVsReportRatePerLevelDeception.png', bbox_inches='tight')
...
def timeline_unique_user_clicks_all_levels(reportData):
    ...
    rects1 = plt.bar(index, intervalpercent, bar_width,
                     color=BLUE,  # blue
    ...
    for rect in rects1:
        height = rect.get_height()
        plt.text(rect.get_x() + rect.get_width() / 2.0, 1.01 * height,
                 '%.0f' % height + "%", ha="center", va="bottom", fontsize=8)
...
def time_first_click_report(reportData):
    ...
    # Establishes the interval bars
    rects1 = plt.bar(index, firstClicks, bar_width,
                     color=BLUE,  # blue
                     label='Time to First Click')
    
    rects2 = plt.bar(index + bar_width, firstReport, bar_width,
                     color=DARK_RED,  # Dark Red
    ...
    for rect in rects1 + rects2:
        ...
        if height != 0:
            ...
            str = '{:02d}:{:02d}:{:02d}'.format(int(hours), int(minutes),
                                                int(seconds))
        else:
            str = "N/A"
        
        plt.text(rect.get_x() + rect.get_width() / 2.0, 1.01 * height, str,
...
def click_rate_based_deception_indicators(reportData):
    ...
    indicators["behavior"]["indicators"] = ["Greed", "Fear", "Curiosity",
                                            "Duty or Obligation"]
    ...
    indicators["sender"]["indicators"] = ["Authoritative - Federal/High-Level",
    ...
    indicators["appearance"]["indicators"] = ["Graphics - Spoofed or HTML",
                                              "Link/Domain - Spoofed or Hidden",
                                              "Link/Domain - Fake",
                                              "Proper Grammar",
                                              "Decent Grammar", "Poor Grammar"]
    ...
    behaviorPlot = plt.subplot2grid((9, 1), (0, 0), colspan=1,
                                    rowspan=2)  # subplot(4,1,1)
    behaviorPlot.set_ylabel("Behavior", fontsize=10, labelpad=87,
                            fontweight='bold')
    behaviorPlot.set_xlim(0, percent_max)
    behaviorPlot.barh(np.arange(4), indicators["behavior"]["click_rates"],
                      align="center", color="#755998")
    behaviorPlot.set_yticks(np.arange(4))
    behaviorPlot.set_yticklabels(indicators["behavior"]["indicators"],
                                 fontsize=10)
    ...
    for i, v in enumerate(indicators["behavior"]["click_rates"]):
        behaviorPlot.text(v + .1, i, str(v) + "%", va="center", fontsize=10)
    ...
    relevancyPlot = plt.subplot2grid((9, 1), (2, 0), colspan=1,
                                     rowspan=1)  # subplot(4,1,2)
    relevancyPlot.set_ylabel("Relevancy", fontsize=10, labelpad=114,
                             fontweight='bold')
    relevancyPlot.xaxis.set_major_formatter(ticker.PercentFormatter(xmax=100))
    relevancyPlot.set_xlim(0, percent_max)
    relevancyPlot.barh(np.arange(2), indicators["relevancy"]["click_rates"],
                       align="center", color="#4576B5")
    relevancyPlot.set_yticks(np.arange(2))
    relevancyPlot.set_yticklabels(indicators["relevancy"]["indicators"],
                                  fontsize=10)
    ...
    for i, v in enumerate(indicators["relevancy"]["click_rates"]):
        relevancyPlot.text(v + .1, i, str(v) + "%", va="center", fontsize=10)
    ...
    senderPlot = plt.subplot2grid((9, 1), (3, 0), colspan=1,
                                  rowspan=3)  # subplot(4,1,3)
    ...
    senderPlot.barh(np.arange(6), indicators["sender"]["click_rates"],
                    align="center", color="#F68B3E")
    ...
    for i, v in enumerate(indicators["sender"]["click_rates"]):
        senderPlot.text(v + .1, i, str(v) + "%", va="center", fontsize=10)
    ...
    appearancePlot = plt.subplot2grid((9, 1), (6, 0), colspan=1,
                                      rowspan=3)  # subplot(4,1,4)
    appearancePlot.set_ylabel("Appearance", fontsize=10, labelpad=11,
                              fontweight='bold')
    appearancePlot.xaxis.set_major_formatter(ticker.PercentFormatter(xmax=100))
    appearancePlot.set_xlim(0, percent_max)
    appearancePlot.barh(np.arange(6), indicators["appearance"]["click_rates"],
                        align="center", color="#B7B7B7")
    appearancePlot.set_yticks(np.arange(6))
    appearancePlot.set_yticklabels(indicators["appearance"]["indicators"],
                                   fontsize=10)
    ...
    for i, v in enumerate(indicators["appearance"]["click_rates"]):
        appearancePlot.text(v + .1, i, str(v) + "%", va="center", fontsize=10)
...
def unique_total_click_report_results_by_level(reportData):
    ...
    rects1 = plt.bar(index, unique_clicks, bar_width,
                     edgecolor='white', color=BLUE,  # Blue
                     label='Unique Clicks', align='edge')
    
    rects2 = plt.bar(index + bar_width, total_clicks, bar_width,
                     edgecolor='white', color=DARK_RED,  # Dark Red
                     label='Total Clicks', align='edge')
    
    # Establishes the User Reports bars
    rects3 = plt.bar(index + (bar_width * 2), user_reports, bar_width,
                     edgecolor='white', color=GREEN,  # Green
                     label='User Reports', align='edge')
...
def breakdown_multiple_clicks_by_level(reportData):
    ...
    for numlevel, value1 in reportData["Level"].items():
        one_click.append(
            reportData['Level'][numlevel]['User_Click_Summary']['1 time'])
    ...
    # Establishes the unique clicks bars
    rects1 = plt.bar(index, one_click, bar_width,
                     edgecolor='white', color='#4576B5',  # Blue
                     label='1 Click', align='edge')
    
    rects2 = plt.bar(index + bar_width, two_3_clicks, bar_width,
                     edgecolor='white', color='#B84644',  # Dark Red
                     label='2-3 Clicks', align='edge')
    
    rects3 = plt.bar(index + bar_width * 2, four_5_clicks, bar_width,
                     edgecolor='white', color='#91B24F',  # Green
                     label='4-5 Clicks', align='edge')
    
    rects4 = plt.bar(index + bar_width * 3, six_10_clicks, bar_width,
                     edgecolor='white', color='#755998',  # purple
                     label='6-10 Clicks', align='edge')
    
    rects5 = plt.bar(index + bar_width * 4, more_10_clicks, bar_width,
                     edgecolor='white', color='#42A2BF',  # Light Blue
                     label='>10 Clicks', align='edge')
    ...
    for rect in rects1 + rects2 + rects3 + rects4 + rects5:
        height = rect.get_height()
        plt.text(rect.get_x() + rect.get_width() / 2.0, 1.01 * height, height,
                 ha="center", va="bottom", fontsize=8)
    
    plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.2),
               frameon=False, ncol=5, prop={'size': 8})
...
def clicking_user_timeline(reportData):
    ...
    for xLists in [levelOne_x, levelTwo_x, levelThree_x, levelFour_x,
                   levelFive_x, levelSix_x]:
    ...
    plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.3),
               frameon=False, ncol=6, fontsize=8)
    # Hide the right and top spines
...
def count_unique_clickes_by_office_per_level(reportData):
    ...
    # Establishes the unique clicks bars
    rects1 = plt.bar(lvl1, level_one, bar_width,
                     edgecolor='white', color=BLUE,  # Blue
                     label='Level 1', align='edge')
    
    rects2 = plt.bar(lvl2, level_two, bar_width,
                     edgecolor='white', color=DARK_RED,  # Dark Red
                     label='Level 2', align='edge')
    
    rects3 = plt.bar(lvl3, level_three, bar_width,
                     edgecolor='white', color=GREEN,  # Green
                     label='Level 3', align='edge')
    
    rects4 = plt.bar(lvl4, level_four, bar_width,
                     edgecolor='white', color=PURPLE,  # Purple
                     label='Level 4', align='edge')
    
    rects5 = plt.bar(lvl5, level_five, bar_width,
                     edgecolor='white', color=LIGHT_BLUE,  # Light Blue
                     label='Level 5', align='edge')
    
    rects6 = plt.bar(lvl6, level_six, bar_width,
                     edgecolor='white', color=ORANGE,  # Orange
                     label='Level 6', align='edge')
    ...
    plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.2),
               frameon=False, ncol=6, prop={'size': 8})
...
def count_unique_clicks_by_level_per_office(reportData):
    ...
    for label, data in reportData["Labels"].items():
        ...
        labelData[count].append(
            reportData['Labels'][label]["1"]["Unique_Clicks"])
        labelData[count].append(
            reportData['Labels'][label]["2"]["Unique_Clicks"])
        labelData[count].append(
            reportData['Labels'][label]["3"]["Unique_Clicks"])
        labelData[count].append(
            reportData['Labels'][label]["4"]["Unique_Clicks"])
        labelData[count].append(
            reportData['Labels'][label]["5"]["Unique_Clicks"])
        labelData[count].append(
            reportData['Labels'][label]["6"]["Unique_Clicks"])
        count += 1
    ...
    rects = list()
    for num in range(numLabels):
        rect = plt.bar(label[num], labelData[num + 1], bar_width,
                       edgecolor='white', color=colorList[num],
                       label=label_names[num], align='edge')
        rects.append(rect)
    
    # Sets x labels.
    plt.xticks([r + bar_width * 2.5 for r in range(len(labelData[1]))],
    ...
    plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.1),
               frameon=False, ncol=3, prop={'size': 8})
...
def perc_total_unique_clicks_belong_office_by_level(reportData):
    ...
    plt.tight_layout()
    plt.savefig('figures/PercTotalUniqueClicksBelongOfficeByLevel.png', bbox_inches='tight')
