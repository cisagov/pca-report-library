...
def graph_builder(assessment_ID, labels):
    ...
    if success:
        ...
        if labels:
            count_unique_clicks_by_office_per_level(reportData)
            percentage_office_clicked_by_level(reportData)
            count_unique_clicks_by_level_per_office(reportData)
            percent_total_unique_clicks_belong_office_by_level(reportData)
...
def unique_user_click_rate_vs_report_rate_per_level_deception(reportData):
    ...
    if userReportProvided and max(ratio) <= 2.0:
        ...
        to_avoid = rects1_label_buffer + rects2_label_buffer
        adjust_text(
            ratio_text,
...
def timeline_unique_user_clicks_all_levels(reportData):
    """Create Time line of Unique User Clicks Across All Levels graph."""
    # data to Plot
...
def time_first_click_report(reportData):
    """Create Time to First Click (HH:MM:SS) graph."""
    # data to plot
    n_groups = 6
    firstClicks = list()
    firstReport = list()
...
def click_rate_based_deception_indicators(reportData):
    """Create Click Rates Based on Deception Indicators graph."""
    indicators = dict()
    click_rate = list()
    click_rates = list()
...
def unique_total_click_report_results_by_level(reportData):
    """Create Unique Click, Total Click, and Report Results by Level graph."""
    # data to plot
    n_groups = 6
    unique_clicks = list()
    total_clicks = list()
...
def breakdown_multiple_clicks_by_level(reportData):
    """Create Breakdown of Multiple Clicks by Level graph."""
    # data to plot
    n_groups = 6
    one_click = list()
    two_3_clicks = list()
    ...
    # Pulls the number of clickers that clicked onces from the Report Data
    for numlevel, value1 in reportData["Level"].items():
        one_click.append(reportData["Level"][numlevel]["User_Click_Summary"]["1 time"])

    # Pulls the number of clickers that clicked 2 to 3 times from the Report Data
    for numlevel, value1 in reportData["Level"].items():
        two_3_clicks.append(
            reportData["Level"][numlevel]["User_Click_Summary"]["2-3 times"]
        )

    # Pulls the number of clickers that clicked 4 to 5 times from the Report Data
    for numlevel, value1 in reportData["Level"].items():
        four_5_clicks.append(
            reportData["Level"][numlevel]["User_Click_Summary"]["4-5 times"]
        )

    # Pulls the number of clickers that clicked 6 to 10 times from the Report Data
    for numlevel, value1 in reportData["Level"].items():
...
def clicking_user_timeline(reportData):
    """Create Time line of Unique Clicks graph."""
    # data to plot
...
def count_unique_clicks_by_office_per_level(reportData):
    """Create Clicks by Level for offices graph."""
    # data to plot
    label_names = list()
    level_one = list()
    level_two = list()
...
def percentage_office_clicked_by_level(reportData):
    """Create Click percent by Level for offices graph."""
    # data to plot
    label_names = list()
    level_one = list()
    level_two = list()
...
def count_unique_clicks_by_level_per_office(reportData):
    """Create Click by Office for level graph."""
    # data to plot
    label_names = list()
    labelData = dict()
    labelData[1] = list()
...
def percent_total_unique_clicks_belong_office_by_level(reportData):
    """Create Percent of All clicks by Office for level graph."""
    # data to plot
    label_names = list()
    labelData = dict()
    labelData[1] = list()
